...
This library has been duplciated and cleaned up to be included in p2llvm's libp2. 
It removes the portability of the library,but the improved readability 
(and editability) is worth it. The core implementations are left untouched,
only naming and formatting to work better with how P2's library needs to be structured.

The primrary changes are:

- remove the multi-target support and be only for P2
- remove quad precision (long double) and ti (128 bit int) support, as we won't support it
- remove trapping functions, as those don't make sense for us

Features can always be re-added by pulling them in from the original compiler-rt.
